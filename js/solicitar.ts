...
function capturarInfoDispositivo(): any {
  return {
...
document.addEventListener("DOMContentLoaded", function() {
  const autorizacaoForm = document.getElementById("autorizacao-form") as HTMLFormElement;
  const alertMessage = document.getElementById("alert-message") as HTMLDivElement;
  
  // Função para validar o formulário
  function validarFormulario(): boolean {
    const dataSaidaStr = (document.getElementById("data_saida") as HTMLInputElement).value;
    const horaSaidaStr = (document.getElementById("horario_saida") as HTMLInputElement).value;
    const dataRetornoStr = (document.getElementById("data_retorno") as HTMLInputElement).value;
    const horaRetornoStr = (document.getElementById("horario_retorno") as HTMLInputElement).value;
  ...
  function mostrarMensagemConfirmacao(numeroAutorizacao: string): void {
    ...
    document.getElementById("btn-copiar-codigo")!.addEventListener("click", function() {
      navigator.clipboard.writeText(numeroAutorizacao).then(function() {
        const btnCopiar = document.getElementById("btn-copiar-codigo") as HTMLButtonElement;
  ...
  autorizacaoForm.addEventListener("submit", async function(e: Event) { // Adicionado async
    e.preventDefault();
    ...
    try {
        const resultadoSalvar = await firebaseService.salvarDocumento("solicitacoes", formData, solicitacaoId);
