...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (!window.AutorizacaoService || !window.firebaseService) {
        ...
        if (solicitacoesPendentesContainer) solicitacoesPendentesContainer.innerHTML = '<p class="text-danger text-center">Erro ao carregar serviços. Tente recarregar a página.</p>';
        if (historicoAprovacoesContainer) historicoAprovacoesContainer.innerHTML = '<p class="text-danger text-center">Erro ao carregar serviços.</p>';
        return;
    }
    ...
    function renderizarSolicitacoes(container, solicitacoes, tipo) {
        ...
        if (solicitacoes.length === 0) {
            container.innerHTML = `<p class="text-center">Nenhuma solicitação encontrada${tipo === 'pendentes' ? ' pendente' : ''} para sua categoria.</p>`;
            return;
        }
    ...
    async function carregarDadosDashboard() {
        ...
        try {
            ...
            // 2. Buscar dados do supervisor (incluindo categoria)
            ...
            if (!supervisorDoc.sucesso || !supervisorDoc.dados || !supervisorDoc.dados.categoria) {
                // Tratar caso de supervisor sem categoria definida
                // Poderia ser uma string ou um array de strings
                supervisorCategoria = null; // Ou um valor padrão, ou mostrar todas?
                console.warn(`Supervisor ${supervisorUid} não tem categoria definida no Firestore. Exibindo todas as solicitações.`);
                // Por segurança, vamos impedir a visualização se a categoria não estiver definida
                 throw new Error('Categoria do supervisor não definida no perfil. Contate o administrador.');
            }
            supervisorCategoria = supervisorDoc.dados.categoria; // Assume que é uma string única por enquanto
            console.log("Supervisor logado pertence à categoria:", supervisorCategoria);

            // 3. Buscar TODAS as solicitações (o filtro será aplicado no serviço)
            // Passar a categoria como filtro
            const filtros = { categoria: supervisorCategoria };
